...
input_path = '/Volumes/TOSHIBA_EXT/NECST/DeepHistReg/dataset' # ANHIR data path 
output_path = '/Volumes/TOSHIBA_EXT/NECST/DeepHistReg/output' # Output path
    ...
    for root, dirs, files in os.walk(input_path):
        for file in files:
            if ".jpg" in file.lower() or ".png" in file.lower():
                ...
                input_file_path = os.path.join(root, file)
                output_file_path = os.path.join(root.replace(original, to_replace), file.replace(".csv", "_output.csv"))  # Adjust output filename
                print("Current input path:" , input_file_path)
                print("Current output path: ", output_file_path)
                if not os.path.isdir(os.path.dirname(output_file_path)):
                    os.makedirs(os.path.dirname(output_file_path))
